...
{
    stack_byte(stack[threadIdx.x].i) &= ~(STACK_DIR_MASK << stack_ofs(stack[threadIdx.x].i));
...
#define state_tile_get(i)                                                      \
    ((state[threadIdx.x].tile & (STATE_TILE_MASK << state_tile_ofs(i))) >> state_tile_ofs(i))
#define state_tile_set(i, val)                                                 \
    do                                                                         \
    {                                                                          \
        state[threadIdx.x].tile &= ~((STATE_TILE_MASK) << state_tile_ofs(i));               \
        state[threadIdx.x].tile |= ((unsigned long long) val) << state_tile_ofs(i);         \
    } while (0)
...
state_move_with_limit(Direction dir, unsigned int f_limit)
{
	...
	int new_h_value = state[threadIdx.x].h_value + H_DIFF(opponent, new_empty, dir);
