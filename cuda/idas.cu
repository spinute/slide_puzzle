...
__device__ static unsigned char get_tile(idx_t i, idx_t j){return state.tile[i][j];}
...
__device__ state_tile_fill(const unsigned char v_list[STATE_WIDTH*STATE_WIDTH])
{
	int cnt = 0;
...
__device__ static inline bool state_is_goal(State state) { return state.h_value == 0; }

__device__ inline static bool state_left_movable(State state) { return state.i != 0; }
__device__ inline static bool state_down_movable(State state) { return state.j != STATE_WIDTH - 1; }
__device__ inline static bool state_right_movable(State state) { return state.i != STATE_WIDTH - 1; }
__device__ inline static bool state_up_movable(State state) { return state.j != 0; }
...
{
	long int rv = strtol(str, end_ptr);
	errno = 0;
