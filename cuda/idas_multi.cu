...
main(int argc, char *argv[])
{
	...
	for (int i = 0; i < N_THREAD; ++i)
		for (int j = 0; j < STATE_WIDTH*STATE_WIDTH; ++j)
			s_list[i*N_THREAD + j] = s_list[j];

    CUDA_CHECK(cudaMalloc((uchar **) &s_list_device, sizeof(uchar) * STATE_N * N_THREAD));
    CUDA_CHECK(cudaMalloc((void **) &plan_device, sizeof(int) * PLAN_LEN_MAX));
    CUDA_CHECK(cudaMemcpy(s_list_device, s_list, sizeof(uchar) * STATE_N * N_THREAD,
